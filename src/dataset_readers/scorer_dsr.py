...
class ScorerDatasetReader(torch.utils.data.Dataset):
    ...
    def get_fields(self, entry):
        ...
        return test_questions, test_answer_strs, test_label, demonstration

    def text_to_instance_choice(self, entry):
        """
        multiple-choice question
        """
        test_questions, test_answers, test_label, demonstration = self.get_fields(entry)  
        ...
        for i in range(len(test_questions)):
            ...
            tokenized_example = self.tokenizer.encode_plus(
                demonstration, truncation=False, return_tensors="pt"
            )
            shape = tokenized_example.input_ids.squeeze().shape
            b = int(shape[0])

            locate_prompt.append(b)
