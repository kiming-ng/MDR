...
from dpr.data.retriever_data_score import Scorer
...
def main(cfg: DictConfig):    
    ...
    # get questions & answers given the TASK
    questions = []
    question_answers = []
    meta_data=[]
    ...
    questions_tensor = retriever.generate_question_vectors( # (testset_size, hidden_dim)
    ...
    # index all passages
    ctx_files_patterns = cfg.encoded_ctx_files # pre-encoded prompt pool
    ...
    # use well-trained retriever to retrieve topk demostrations for each sample in the testset
    ...
    # get each prompt's eigenvalue if apply MDR
    if cfg.method.startswith("mdr"):
        accelerator = Accelerator()
        with open(cfg.score.new_prompt_pool_path, "r") as f:
            all_data = json.load(f)
        model = AutoModelForCausalLM.from_pretrained(pretrained_model_name_or_path=cfg.score.dataset_reader.model_name, cache_dir=cfg.score.dataset_reader.cache_dir)
        model = model.half().to(accelerator.device)
        for t in trange(len(top_ids_and_scores)):
            (candidate_list, score) = top_ids_and_scores[t]
            init_eigen, init_loss = [], []     
            for prompt_idx in candidate_list:
                if "eigen" not in  all_passages[prompt_idx].meta_data: # initial eigen value if the prompt has not been score yet
                    all_passages[prompt_idx].meta_data["eigen"] = 0
                    all_passages[prompt_idx].meta_data["loss"] = 0
            init_eigen = [all_passages[idx].meta_data["eigen"] for idx in candidate_list] 
            init_loss = [all_passages[idx].meta_data["loss"] for idx in candidate_list]
            prompt_list = [all_passages[idx] for idx in candidate_list if all_passages[idx].meta_data["eigen"] == 0] # prompts that has not been score yet
            if len(prompt_list) != 0:
                scorer = Scorer(cfg.score, model, accelerator, prompt_list)
                metadata_list = scorer.forward()                     
                eigen = [x["eigen"] for x in metadata_list]
                loss = [x["loss"] for x in metadata_list]
                tot=0
                for ii in range(len(init_eigen)):
                    if init_eigen[ii] == 0 and init_loss[ii]==0:
                        init_eigen[ii] = eigen[tot]
                        init_loss[ii] = loss[tot]
                        all_passages[candidate_list[ii]].meta_data["eigen"] = eigen[tot]
                        all_passages[candidate_list[ii]].meta_data["loss"] = loss[tot]
                        all_data[candidate_list[ii]]["eigen"] = eigen[tot]
                        all_data[candidate_list[ii]]["loss"] = loss[tot]
                        tot+=1     
                with open(cfg.score.new_prompt_pool_path, "w") as writer:
                    writer.write(json.dumps(all_data, indent=4) + "\n")    
            mix = [cfg.C * init_eigen[i] + (1-cfg.C) * init_loss[i] for i in range(len(init_loss))]
            my_dict = dict(zip(candidate_list, mix))
            element_sorted, eigen_sorted = map(list, zip(*sorted(my_dict.items(),reverse=False,key=lambda x:x[1])))
            top_ids_and_scores[t]=(element_sorted, eigen_sorted)
...
if __name__ == "__main__":
    ...
    main()
    etime = time.time()
    logger.info("Retrieval Time Cost: {}".format(etime - stime))
