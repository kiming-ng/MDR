...
test_cluster_map = {
    "close_qa": ["natural_questions", "arc_c", "arc_e","trivia_qa"],
    "common_reason": ["copa", "piqa", "hellaswag"],
    ...
    "sentiment": ["yelp", "sentiment140", "sst2"],
    "struct2text": ["common_gen", "e2e_nlg", "dart"],
    "summarize": ["aeslc", "ag_news", "gigaword", "python", "java"],
    "translate": ["wmt14", "wmt16"],
...
class BaseTask(object):
    ...
    def load_data_split(self, dataset, ds_size=None, split="train"):
        assert split in ["train", 'validation', 'test', 'train_r1', 'test_r1']
        if split in ["train", "train_r1", "test", "validation", 'test_r1']:
            ds_size = TEST_SIZE
        ...
        if self.balance_class:  # balance data examples sampled from each class
            ...
            x = data[:ds_size]
        if split in ["train", "train_r1", "test", "validation"]:
            random.shuffle(data)
            x = data[:TEST_SIZE]
        return x
...
class Amazon(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("KaiLv/UDR_Amazon", cache_dir=cache_dir)
        if split == "train":
    ...
    def get_templates(self):
        return [
                ("Sentiment of the sentence: {sentence} It is ", "{answer}"),
            ]
    ...
    def get_answers(self, entry):
        answers = ["terrible", "bad", "okay", "good", "great"]
        return answers
...
class Java(BaseTask):
    ...
    def filter(self, entry):
        text = " ".join([entry["question"], entry["target"]])
        text = re.sub(r"\\\"", "", text)
        text = re.sub(r"\\n\\n", " ", text)
        return len(text.split(" ")) <= 188

    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset('KaiLv/UDR_Java', cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test"
    ...
    def get_templates(self):
        return [
                ("Comment on the code. Code: {question}", "{answer}"),
                ("Summary the code. Code: {question}", "{answer}"),
            ]
...
class Python(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset('KaiLv/UDR_Python', cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test"
    ...
    def get_templates(self):
        return [
                ("Comment on the code. Code: {question}", "{answer}"),
                ("Summary the code. Code: {question}", "{answer}"),
                ("What does the following code represents? Code: {question}", "{answer}"),
                ("Code: {question} Summarization: ", "{answer}"),
            ]
...
@task_map.add("trivia_qa")
class Trivia_qa(BaseTask):
    def __init__(self):
        super().__init__()
        self.class_num = 1
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("trivia_qa", "rc.web.nocontext", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "validation"
    ...
    def filter(self, entry):
        if len(entry["answer"]["aliases"]) == 0:
            return False
        else:
            answers = entry["answer"]["aliases"][0]
            text = " ".join([entry["question"], answers])
            text = re.sub(r"\\\"", "", text)
            text = re.sub(r"\\n\\n", " ", text)
...
@task_map.add("wmt14")
class Wmt14(BaseTask):
    def __init__(self):
        super().__init__()
        self.class_num = 1
        self.metric = "bleu"
        self.balance_class = False
        self.cluster = "translate"

    def filter(self, entry):
        text = " ".join([entry["translation"]["cs"], entry["translation"]["en"]])
        text = re.sub(r"\\\"", "", text)
        text = re.sub(r"\\n\\n", " ", text)
        return len(text.split(" ")) <= 200

    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset('wmt14', 'cs-en', cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test"
    ...
    def get_templates(self):
        return [
                ("\"{sent1}\" Translate to {lang2}", "{sent2}"),
                ("\"{sent2}\" Translate to {lang1}", "{sent1}"),
                ("{sent2} Could you please translate this to {lang1}?", "{sent1}"),
                ("{sent2} Translate this to {lang1}?", "{sent1}"),
                ("Translate to {lang2}: \"{sent1}\"", "{sent2}"),
                ("Translate the following sentence to {lang2}: \"{sent1}\"", "{sent2}"),
                ("How is \"{sent1}\" said in {lang2}?", "{sent2}"),
                ("Translate \"{sent1}\" to {lang2}?", "{sent2}"),
            ]

    def get_question(self, entry):
        sent1 = entry["translation"]["cs"]
        sent2 = entry["translation"]["en"]
        lang2 = "English"
        template = self.get_template(entry)
        return template.replace("{sent1}", sent1).replace("{lang2}", lang2)
...
class MR(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("KaiLv/UDR_MR", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test"
...
class Subj(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("KaiLv/UDR_Subj", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test"
    ...
    def get_templates(self):
        return [
            ("Subjectivity status of the sentence: \"{sentence}\"", "It is {answer}"),
            ("Judge the subjectivity of the following sentences: {sentence}", "It is {answer}"),
        ]
...
class Wmt16(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset('wmt16', 'cs-en', cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test"
    ...
    def get_templates(self):
        return [
                ...
                ("{sent2} Which language is this?", "{lang2}"),
            ]
    ...
    def get_question(self, entry):
        ...
        lang2 = "English"
        template = self.get_template(entry)
        return template.replace("{sent1}", sent1).replace("{sent2}", sent2).replace("{lang2}", lang2)
...
class Imdb(BaseTask):
    ...
    def filter(self, entry):
        ...
        text = re.sub(r"\\\"", "", text)
        text = re.sub(r"\\n\\n", " ", text)
        return len(text.split(" ")) <= 240

    def get_templates(self):
        return [
                ("\"{text}\" What is the sentiment of this review?", "{answer}"),
                ("\"{text}\" Would you say this review is positive or negative?", "{answer}"),
                ("\"{text}\" How would you describe the sentiment of this review?", "{answer}"),
                ("\"{text}\" Is the sentiment of this review positive or negative?", "{answer}"),
                ("\"{text}\" Did this review think positively or negatively of the movie?", "{answer}"),
                ("Please tell me the sentiment of the following review: \"{text}\"", "{answer}"),
                ("Determine the sentiment: \"{text}\"", "{answer}"),
    ...
    def get_question(self, entry):
        sentence = entry["text"]
        template = self.get_template(entry)
        return template.replace("{text}", sentence)
...
@task_map.add("rotten_tomatoes")
class ROTTEN_TOMATOES(BaseTask):
    def __init__(self):
        super().__init__()
        self.class_num = 2
        self.metric = "simple_accuracy"
...
class WIC(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("stjokerli/TextToText_wic_seqio", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "validation"
    ...
    def get_templates(self):
        return [
            ("{sentence1} {sentence2} Does the word \"{word}\" mean the same thing in the above two sentences?", "{answer}"),
            ("Sentence 1: {sentence1} Sentence 2: {sentence2} Does {word} mean the same thing in these two sentences?", "{answer}"),
            ("Here is one sentence: {sentence1} Here is another sentence: {sentence2} Does the term {word} mean the same thing in both these sentences?", "{answer}"),
            ("In these two sentences (1) {sentence1} (2) {sentence2}, does the word {word} mean the same thing?", "{answer}"),
            ("Does word \"{word}\" have the same meaning in the following two sentences? {sentence1} {sentence2}", "{answer}"),
            ("Is the word \"{word}\" used in the same way in the following two sentences? {sentence1} {sentence2}", "{answer}"),
            ("Does the word \"{word}\" have the same definition in the next two sentences? {sentence1} {sentence2}", "{answer}"),
            ("Is {word} used to mean the same thing in the next two sentences?{sentence1} {sentence2}", "{answer}"),
            ("Does \"{word}\" mean the same thing in these two sentences? {sentence1} {sentence2}", "{answer}"),
            ("Does the word \"{word}\" mean the same thing in \"{sentence1}\" and \"{sentence2}\"?", "{answer}"),
        ]
 
    def get_question(self, entry):
        template = self.get_template(entry)
        text = entry["inputs"]
        match = re.search(r"sentence1:\s*(.*?)(?:\s*sentence2:|$)", text)
        sentence1 = match.group(1).strip()
        match = re.search(r"sentence2:\s*(.*?)(?:\s*word:|$)", text)
        sentence2 = match.group(1).strip()
        match = re.search(r"word:\s*(\w+)", text)
        word = match.group(1)
        template = template.replace("{sentence1}", sentence1)
        template = template.replace("{sentence2}", sentence2)
        return template
    ...
    def get_label(self, entry):
        ...
        if ans == 'True':
            ...
            raise 'Unkonwn Target: {}'.format(ans)
        return label
...
class Anli(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("anli", cache_dir=cache_dir)
        if split == "train_r1":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test_r1"
            return self.load_data_split(dataset, split='train_r1')

    def get_templates(self):
        return [
            ("{context} Based on the paragraph above can we conclude that \"{hypothesis}\"?", "{answer}"),
            ("{context} Based on that paragraph can we conclude that this sentence is true? {hypothesis}", "{answer}"),
            ("{context} Can we draw the following conclusion? {hypothesis}", "{answer}"),
            ("{context} Does this next sentence follow, given the preceding text? {hypothesis}", "{answer}"),
            ("{context} Can we infer the following? {hypothesis}", "{answer}"),
            ("Read the following paragraph and determine if the hypothesis is true: {context} Hypothesis: {hypothesis}", "{answer}"),
            ("Read the text and determine if the sentence is true: {context} Sentence: {hypothesis}", "{answer}"),
            ("Can we draw the following hypothesis from the context? Context:{context} Hypothesis: {hypothesis}", "{answer}"),
            ("Determine if the sentence is true based on the text below: {hypothesis} {context}", "{answer}"),
            ("Generate a context and a hypothesis.", "Context: {context} Hypothesis: {hypothesis}"),
        ]
    ...
    def get_question(self, entry):
        ...
        template = self.get_template(entry)
        return template.replace("{context}", premise).replace("{hypothesis}", hypothesis)
...
class CoLA(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("linxinyuan/cola", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test"
    ...
    def get_templates(self):
        return [
            # UDR Template
            ("The grammaticality of this sentence: {sentence}. It is ", "{answer}")
            ("Sentence: \"{sentence}\" Would a linguist rate this sentence to be acceptable linguistically?", "{answer}"),
            ("{sentence} How would you consider the linguistic integrity of the preceding sentence?", "{answer}"),
            ("Sentence: \"{sentence}\" Is this sentence grammatically correct?", "{answer}"),
            ("Test sentence: \"{sentence}\" Is this test sentence a correct grammatical English sentence?", "{answer}"),
            ("Sentence: \"{sentence}\" Would a linguist rate this sentence to be acceptable linguistically?", "{answer}"),
            ("Is the following sentence linguistically acceptable? {sentence}", "{answer}"),
            ("Would the following sentence, by the strictest standards, be considered correct by a linguist? {sentence}", "{answer}"),
            ("Is the next sentence syntactically and semantically acceptable? {sentence}", "{answer}"),
            ("Would a linguist find the following sentence to be a valid English sentence grammatically? {sentence}", "{answer}"),
    ...
    def get_answers(self, entry):
        answers = ["not grammatical", "grammatical"]
        return answers
...
class Trec(BaseTask):
    ...
    def get_templates(self):
        return [
                ("What type of thing is the question \"{text}\" asking about?", "{answer}"),
                ("Is the question \"{text}\" asking about an entity, an abbreviation, a description, a human, a location, or a numeric entity?", "{answer}"),
                ("Would the answer to the question \"{text}\" be an entity, an abbreviation, a description, a human, a location, or a numeric value?", "{answer}"),
                ("What kind of thing would the answer to the question \"{text}\" be an entity, an abbreviation, a description, a human, a location, or a numeric value?", "{answer}"),
                ("What is \"{text}\" asking about?", "{answer}"),
                ("From the following options, what is the question \"{text}\" asking about?", "{answer}"),
                ("\"{text}\" What kind of thing would answer this question?", "{answer}"),
                ("Here is a question: \"{text}\" Would the answer to this question be an entity, an abbreviation, a description, a human, a location, or a numeric value?", "{answer}"),
                ("Q: \"{text}\" Which one of the following options would the answer to this be?", "{answer}"),
                ]
    ...
    def get_answers(self, entry):
        answers = ["ABBR", "ENTY", "DESC", "HUM", "LOC", "NUM"]
        # answers = [" Abbreviation", " Entity", " Description and abstract concept", "Human being", "Location", "Numeric value"]
        return answers

    def get_label(self, entry):
        label = int(entry["coarse_label"])
        return label
...
class Wnli(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("SetFit/wnli", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "validation"
            return self.load_data_split(dataset, split='train')

    def get_templates(self):
        return [
            ("If \"{sentence1}\", can we conclude that \"{sentence2}\"", "{answer}"),
            ("If \"{sentence1}\", does it follow that \"{sentence2}\"", "{answer}"),
            ("If \"{sentence1}\", is \"{sentence2}\" correct?", "{answer}"),
            ("Let's say that \"{sentence1}\" Can we now say that \"{sentence2}\"?", "{answer}"),
            ("\"{sentence1}\" is a true sentence. Does this mean that \"{sentence2}\"?", "{answer}"),
            ("Does \"{sentence2}\" appear to be an accurate statement based on \"{sentence1}\"?", "{answer}"),
            ("Can we conclude that \"{sentence2}\" if the statement \"{sentence1}\" is true?", "{answer}"),
            ("Is it possible to draw the conclusion that \"{sentence2}\" if \"{sentence1}\"?", "{answer}"),
            ("Is \"{sentence2}\" true if \"{sentence1}\"?", "{answer}"),
            ("Sentence 1: \"{sentence1}\" Sentence 2: \"{sentence2}\" Is sentence 2 true, based on sentence 1?", "{answer}"),
        ]
    ...
    def get_answers(self, entry):
        answers = [" not entailment", " entailment"]
        return answers
...
@task_map.add("cosmos_qa")
class CosmosQA(BaseTask):
    def __init__(self):
        super().__init__()
        self.class_num = 4
        self.metric = "simple_accuracy"
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("cosmos_qa", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "validation"
    ...
    def get_templates(self):
        return [
            ("Context: {context} Question {question} Answer: {options_}", "{answer}"),
            ("{context} Question: {question} {options_}", "{answer}"),
            ("{context} {question} {options_}", "{answer}"),
            ("{context} Answer the following question: {question} {options_}", "{answer}"),
            ("{context} Based on the preceding passage, answer the following question {question} {options_}", "{answer}"),
            ("{context} Give answer the following question using evidence from the above passage: {question} {options_}", "{answer}"),
            ("Read the following article and answer the question.{context} {question} {options_}", "{answer}"),
            ("Answer the question about text: {context} {question} {options_}", "{answer}"),
            ("Write a question about the article {context}", "{question}"),
            ("{context} Generate a question about the above context.", "{question}"),
        ]

    def get_question(self, entry):
        context = entry["context"]
        question = entry["question"]
        opts = "A." + entry["answer0"] + " B." + entry["answer1"] + " C." + entry["answer2"] + " D." + entry["answer3"]
        template = self.get_template(entry)        
        template = template.replace("{context}", context)
        template = template.replace("{question}", question)
        template = template.replace("{options_}", opts)
        return template
...
class Samsum(BaseTask):
    ...
    def filter(self, entry):
        text = "".join([entry["dialogue"], entry["summary"]])
        no_unk = "UNK" not in text
        no_hashtag = "#" not in text
        return no_unk and no_hashtag

    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("samsum", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test"
...
@task_map.add("squad_v2")
class Squad_v2(BaseTask):
    def __init__(self):
        super().__init__()
    ...
    # add filter to fix max_length problem
    def filter(self, entry):
        text = entry["context"]
        text = re.sub(r"\\\"", "", text)
        text = re.sub(r"\\n\\n", " ", text)
        return len(entry["answers"]["text"]) > 0 and len(text) > 0 and len(text.split(" ")) <= 200
    ...
    def get_templates(self):
        return [
            ("{title}: {context} Please answer a question about this article. If the question is unanswerable, say \"unanswerable\". {question}", "{answer}"),
            ("Read this and answer the question. If the question is unanswerable, say \"unanswerable\". {context} {question}", "{answer}"),
            ("What is a question about this article? If the question is unanswerable, say \"unanswerable\". {context} {question}", "{answer}"),
            ("{context} {question} (If the question is unanswerable, say \"unanswerable\")", "{answer}"),
            ("{context} Try to answer this question if possible (otherwise reply \"unanswerable\"): {question}", "{answer}"),
            ("{context} If it is possible to answer this question, answer it for me (else, reply \"unanswerable\"): {question}", "{answer}"),
            ("{context} Answer this question, if possible (if impossible, reply \"unanswerable\"): {question}", "{answer}"),
            ("Read this: {context} {question} What is the answer? (If it cannot be answered, return \"unanswerable\")", "{answer}"),
            ("Read this: {context} Now answer this question, if there is an answer (If it cannot be answered, return \"unanswerable\"): {question}", "{answer}"),
            ("{context} Is there an answer to this question (If it cannot be answered, say \"unanswerable\"): {question}", "{answer}"),
        ]
...
class Com_E(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("KaiLv/UDR_ComE", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "test"
    ...
    def get_question(self, entry):
        question = entry["question"] + "?"
        template = self.get_template(entry)
        return template.replace("{context}", question)
    ...
    def get_label(self, entry):
        if entry['label'].split('.')[0] == 'A':
            label = 0
        elif entry['label'].split('.')[0] == 'B':
            label = 1
        elif entry['label'].split('.')[0] == 'C':
            label = 2
        else:
...
class Sst5(BaseTask):
    ...
    def get_dataset(self, split=None, ds_size=None, cache_dir=None):
        dataset = load_dataset("SetFit/sst5", cache_dir=cache_dir)
        if split == "train":
            return self.load_data_split(dataset, ds_size=ds_size, split=split)
        else:  
            split = "validation"
    ...
    def get_templates(self):
        # five-class
        return [ 
                ("Review: \"{sentence}\" Is this movie review sentence very positive, positive, neutral, negative or very negative?", "{answer}"),
                ("Short movie review: \"{sentence}\" Did the critic thinking positively or negatively of the movie?", "{answer}"),
                ("Sentence from a movie review: \"{sentence}\" Was the movie seen positively or negatively based on the preceding review?", "{answer}"),
                ("\"{sentence}\" How would the sentiment of this sentence be perceived?", "{answer}"),
                ("Is the sentiment of the following sentence very positive, positive, neutral, negative or very negative? \"{sentence}\"", "{answer}"),
                ("What is the sentiment of the following movie review sentence? \"{sentence}\"", "{answer}"),
                ("Would the following phrase be considered very positive, positive, neutral, negative or very negative? \"{sentence}\"", "{answer}"),
                ("Does the following review have a very positive, positive, neutral, negative or very negative opinion of the movie? \"{sentence}\"", "{answer}"),
            ]
...
class Aeslc(BaseTask):
    ...
    def filter(self, entry):
        ...
        answer = re.sub(r"\n", " ", answer)
        return (
            len(text.split()) > 0 and len(answer.split()) > 0 and len(text.split()) <= 240 and len(answer.split()) <= 256
        )
