...
BiEncoderPassage = collections.namedtuple(
    ...
    "BiEncoderPassage", ["text", "title", "meta_data", "eigen"]
)
...
class UpriseDataset(Dataset):
    ...
    def get_entry(self, entry, eigen_type):
        if eigen_type=='max':
            used_eigen='eigen' # 原max_eigenvalue
        elif eigen_type=='mix':
            used_eigen='mix'
        ...
        positive_cntx = [
            {"demonstration": self.format_example(p_example, self.prompt_setup_type), "eigen": p_example[used_eigen]} # eigen/mix
        ...
        hard_negative_ctxs = [
            {"demonstration": self.format_example(n_example, self.prompt_setup_type), "eigen": n_example[used_eigen]}
        ...
        # get the max eigen in hard negatives
        hard_eigen = sorted(hard_negative_ctxs,key = lambda x: x['eigen'])
        max_hard_eigen = hard_eigen[-1]['eigen']
        ...
        if self.multi_task:
            ...
            negative_cntx = [
                {"demonstration": self.format_example(n_example, self.prompt_setup_type), "eigen": max_hard_eigen}
    ...
    def load_data(self, eigen_type):
        ...
        for entry in raw_data:
            self.data.append(self.get_entry(entry, eigen_type))
    ...
    def __getitem__(self, index) -> BiEncoderSample:
        ...
        def create_passage(ctx: dict):
            return BiEncoderPassage(
                ctx["demonstration"], ctx["title"], None, ctx["eigen"]  # meta_data=None
            )
