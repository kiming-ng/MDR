...
class BiEncoder(nn.Module):
    ...
    def create_biencoder_input2(
        ...
        for sample in samples:
            ...
            sample_ctxs_eigen = [
                ctx.eigen for ctx in all_ctxs
            ]
            all_eigen.extend(sample_ctxs_eigen)
...
class BiEncoderNllLoss(object):
    def calc(
        ...
        if method.startswith("ours"):
            eigen = torch.tensor(all_eigen)
            diagonal = torch.diag(eigen).to(scores.device)
            '''
            # divide eigen
            loc = torch.where(diagonal == 0)   # mind 0
            diagonal[loc]=1
            diagonal_1 = 1/diagonal
            diagonal_1[loc] = 0
            '''
            # multiply eigen
            scores = torch.mm(scores, diagonal)
            
            # remove in batch trick
            top_k = positive_idx_per_question[1]
            new_scores = scores[0, positive_idx_per_question[0]:positive_idx_per_question[0]+top_k].unsqueeze(0)
            for i in range(1,len(positive_idx_per_question)):
                temp = scores[i, positive_idx_per_question[i]:positive_idx_per_question[i]+top_k].unsqueeze(0)
                new_scores = torch.cat((new_scores,temp),0)
        
            positive_idx_per_question = torch.tensor(positive_idx_per_question)
            new_positive_idx_per_question = torch.zeros(positive_idx_per_question.shape, dtype=int)
            
            softmax_scores = F.log_softmax(new_scores, dim=1)
            loss = F.nll_loss(
                softmax_scores,
                new_positive_idx_per_question.to(softmax_scores.device),
                reduction="mean",
            )
        else:
